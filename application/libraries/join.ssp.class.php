...
class JSSP {
	...
	static function order ( $request, $columns, $isJoin = false )
	{
		...
		if ( isset($request['order']) && count($request['order']) ) {
			$orderBy = array();
			$dtColumns = JSSP::pluck( $columns, 'dt' );
		...
		if ( isset($request['search']) && $request['search']['value'] != '' ) {
			...
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) {
					$binding = JSSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
	 ...
	 * in response to an JSSP request, or can be modified if needed before
	...
	{
		$bindings = array();
		$db = JSSP::sql_connect( $sql_details );
		// Build the SQL query string from the request
		$limit = JSSP::limit( $request, $columns );
		$order = JSSP::order( $request, $columns, $joinQuery );
		$where = JSSP::filter( $request, $columns, $bindings, $joinQuery );
		...
		if($joinQuery){
			$col = JSSP::pluck($columns, 'db', $joinQuery);
       ...
       $having
			...
			$query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", JSSP::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
       $having
		...
		$data = JSSP::sql_exec( $db, $bindings,$query);
		// Data set length after filtering
		$resFilterLength = JSSP::sql_exec( $db,
			"SELECT FOUND_ROWS()"
		);
		$recordsFiltered = $resFilterLength[0][0];
		// Total data set length
		$resTotalLength = JSSP::sql_exec( $db,
		...
		return array(
			...
			"data"            => JSSP::data_output( $columns, $data, $joinQuery )
		);
	}
	/**
	...
	static function sql_exec ( $db, $bindings, $sql=null )
	{
		...
		try {
			...
			JSSP::fatal( "An SQL error occurred: ".$e->getMessage() );
		}
		// Return all
